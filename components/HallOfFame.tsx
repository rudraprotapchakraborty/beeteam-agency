...
const awards: Award[] = [
  ...
  {
    url: '/awards/award-6.jpg',
    title: 'Certificate of Recognition',
    outlet: 'Dhaka International Film Festival',
    desc: 'Awarded in recognition of outstanding contribution to the 24th Dhaka International Film Festival.',
    date: 'JAN 2026',
    id: 'DIFF-663',
  },
]
...
export default function HallOfFame() {
  ...
  return (
    <section id="certification" className="relative bg-page py-20 md:py-28 overflow-hidden">
          ...
          {awards.map((award, i) => (
            ...
            >
              <div className="relative aspect-[4/3] overflow-hidden bg-black">
                ...
                <div className="absolute bottom-3 right-3 w-9 h-9 rounded-full bg-[#ffd700] text-black opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center scale-90 group-hover:scale-100">
                  <Maximize2 size={14} strokeWidth={2.5} />
                </div>
              </div>
              <div className="flex flex-col flex-1 p-5 space-y-2.5">
                <div className="font-mono text-[9px] uppercase tracking-[0.28em] text-(--gold-text)">
                  {award.outlet}
                </div>
                <h3 className="text-base font-semibold text-fg leading-tight group-hover:text-(--gold-text) transition-colors">
                  {award.title}
                </h3>
                <p className="text-sm text-muted leading-relaxed line-clamp-2 flex-1">{award.desc}</p>
                <div className="flex justify-between items-center pt-3 border-t border-line">
                  <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-faint">
                    {award.date}
                  </span>
                  <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-muted group-hover:text-(--gold-text) transition-colors font-bold">
                    {t.verify} →
                  </span>
                </div>
              </div>
